...
export default function SidebarLayout({ children }: { children: React.ReactNode }) {
  ...
  const [whatIfOpen, setWhatIfOpen] = useState(
    location.pathname === '/scenario-branching' || location.pathname === '/anti-portfolio' || location.pathname === '/goal-tracker'
  );
  ...
  const whatIfItems = [
    { path: '/goal-tracker', label: t('nav.goalTracker'), icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M3 3v1.5M3 21v-6m0 0l2.77-.693a9 9 0 016.208.682l.108.054a9 9 0 006.086.71l3.114-.732a48.524 48.524 0 01-.005-10.499l-3.11.732a9 9 0 01-6.085-.711l-.108-.054a9 9 0 00-6.208-.682L3 4.5M3 15V4.5" />
      </svg>
    )},
    { path: '/scenario-branching', label: t('nav.scenarios'), icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M7.5 21L3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5" />
      </svg>
    )},
    { path: '/anti-portfolio', label: t('nav.antiPortfolio'), icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
  ...
  const bottomItems = [
    // Financial Lab — premium, direct link (tab-based page)
    ...
    { path: '/calculators', label: t('nav.calculators'), icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
    ...
    // Admin — only visible to whitelisted users
    ...(isAdmin ? [{
    ...
    <>
      ...
      <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
        {mainItems.map((item) => {
          ...
          const isLocked = isFree && item.premium;
          return isLocked ? (
            <button
              key={item.path}
              onClick={() => setShowUpgrade(true)}
              className="w-full flex items-center gap-3 px-3.5 py-3 rounded-xl transition-all duration-200 group relative text-white/25 hover:text-white/40 hover:bg-white/[0.03] cursor-pointer"
            ...
            >
              ...
              {!collapsed && (
                <>
                  ...
                  <span className="ml-auto text-[9px] font-bold bg-amber-500/20 text-amber-400 px-1.5 py-0.5 rounded-md">PRO</span>
                </>
              )}
            </button>
          ) : (
        ...
        {bottomItems.map((item: any) => {
          const isActive = location.pathname === item.path || (item.path === '/financial-lab' && ['/goal-tracker', '/scenario-branching', '/anti-portfolio'].includes(location.pathname));
          const isLocked = isFree && item.premium;
          if (isLocked) {
            return (
              <button
                key={item.path}
                onClick={() => setShowUpgrade(true)}
                className="w-full flex items-center gap-3 px-3.5 py-3 rounded-xl transition-all duration-200 text-white/25 hover:text-white/40 hover:bg-white/[0.03]"
              >
                <span className="flex-shrink-0 opacity-50">{item.icon}</span>
                {!collapsed && (
                  <>
